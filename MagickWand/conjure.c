# 0 "MagickWand/conjure.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "MagickWand/conjure.c"
# 48 "MagickWand/conjure.c"
...
# 51 "MagickWand/conjure.c" 2
# 90 "MagickWand/conjure.c"
static MagickBooleanType ConjureUsage(void)
{
  ...
  ListMagickVersion(
# 106 "MagickWand/conjure.c" 3 4
                   stdout
# 106 "MagickWand/conjure.c"
                         );
...
__attribute__ ((visibility ("default"))) MagickBooleanType ConjureImageCommand(ImageInfo *image_info,
  int argc,char **argv,char **wand_unused_metadata __attribute__((unused)),ExceptionInfo *exception)
{
# 144 "MagickWand/conjure.c"
  char
    filename[4096],
    *option;
...
# 165 "MagickWand/conjure.c" 3 4
 ((void) sizeof ((
# 165 "MagickWand/conjure.c"
...
# 165 "MagickWand/conjure.c" 3 4
 , "MagickWand/conjure.c", 165, __extension__ __PRETTY_FUNCTION__); }))
# 165 "MagickWand/conjure.c"
                                         ;
  
# 166 "MagickWand/conjure.c" 3 4
 ((void) sizeof ((
# 166 "MagickWand/conjure.c"
...
# 166 "MagickWand/conjure.c" 3 4
 , "MagickWand/conjure.c", 166, __extension__ __PRETTY_FUNCTION__); }))
# 166 "MagickWand/conjure.c"
                                                     ;
  
# 167 "MagickWand/conjure.c" 3 4
 ((void) sizeof ((
# 167 "MagickWand/conjure.c"
...
# 167 "MagickWand/conjure.c" 3 4
 , "MagickWand/conjure.c", 167, __extension__ __PRETTY_FUNCTION__); }))
# 167 "MagickWand/conjure.c"
                                            ;
  if (IsEventLogging() != MagickFalse)
    (void) LogMagickEvent(TraceEvent,"MagickWand/conjure.c",__func__,(unsigned long) 169,"...");
  if (argc < 2)
    {
      (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 172,OptionError,
        "MissingArgument","%s","");
      (void) ConjureUsage();
      return(MagickFalse);
    }
  ...
  number_images=0;
  option=(char *) 
# 179 "MagickWand/conjure.c" 3 4
                 ((void *)0)
# 179 "MagickWand/conjure.c"
                     ;
  ...
  if (status == MagickFalse)
    { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 186,ResourceLimitError,"MemoryAllocationFailed","`%s'", GetExceptionMessage(
# 186 "MagickWand/conjure.c" 3 4
   (*__errno_location ())
# 186 "MagickWand/conjure.c"
   )); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); }
                                 ;
    ...
    if (IsCommandOption(option) != MagickFalse)
      {
        ...
        if (LocaleCompare("debug",option+1) == 0)
          {
            ...
            i++;
            if (i == (ssize_t) argc)
              { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 204,OptionError,"MissingArgument","`%s'", option); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
            event=ParseCommandOption(MagickLogEventOptions,MagickFalse,argv[i]);
            if (event < 0)
              { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 207,OptionError,"UnrecognizedEventType","`%s'", argv[i]); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); }
                        ;
        ...
        if (LocaleCompare("duration",option+1) == 0)
          {
            ...
            i++;
            if (i == (ssize_t) argc)
              { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 218,OptionError,"MissingArgument","`%s'", option); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
            if (IsGeometry(argv[i]) == MagickFalse)
              { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 220,OptionError, "InvalidArgument","'%s': %s",option,argv[i]); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
            continue;
          }
        if ((LocaleCompare("help",option+1) == 0) ||
            (LocaleCompare("-help",option+1) == 0))
          {
            if (*option == '-')
              {
                { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); };
                return(ConjureUsage());
              }
            continue;
          }
            ...
            if (*option == '-')
              {
                i++;
                if (i == (ssize_t) argc)
                  { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 239,OptionError,"MissingLogFormat","`%s'", option); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
        ...
        if (LocaleCompare("seed",option+1) == 0)
          {
            ...
            i++;
            if (i == (ssize_t) argc)
              { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 256,OptionError,"MissingArgument","`%s'", option); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
            if (IsGeometry(argv[i]) == MagickFalse)
              { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 258,OptionError, "InvalidArgument","'%s': %s",option,argv[i]); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
            break;
          }
        ...
        if ((LocaleCompare("version",option+1) == 0) ||
          ...
          {
            ListMagickVersion(
# 269 "MagickWand/conjure.c" 3 4
                             stdout
# 269 "MagickWand/conjure.c"
                                   );
            return(MagickTrue);
          }
        ...
        i++;
        if (i == argc)
          { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 278,OptionError,"MissingArgument","`%s'", option); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
        status=SetImageOption(image_info,option+1,argv[i]);
        if (status == MagickFalse)
          { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 281,ImageError,"UnableToPersistKey","`%s'", option); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
        continue;
      }
    ...
    if (status == MagickFalse)
      { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 290,ImageError,"UnableToPersistKey","`%s'", argv[i]); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
    (void) FormatLocaleString(filename,4096,"%s",argv[i]);
    image=ReadImages(image_info,filename,exception);
    CatchException(exception);
    if (image != (Image *) 
# 294 "MagickWand/conjure.c" 3 4
                          ((void *)0)
# 294 "MagickWand/conjure.c"
                              )
      image=DestroyImageList(image);
    status=image != (Image *) 
# 296 "MagickWand/conjure.c" 3 4
                             ((void *)0) 
# 296 "MagickWand/conjure.c"
                                  ? MagickTrue : MagickFalse;
    number_images++;
  }
  if (i != (ssize_t) argc)
    { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 300,OptionError,"MissingAnImageFilename","`%s'", argv[i]); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
  if (number_images == 0)
    { (void) ThrowMagickException(exception,"MagickWand/conjure.c",__func__,(unsigned long) 302,OptionError,"MissingAnImageFilename","`%s'", argv[argc-1]); { image=DestroyImageList(image); for (i=0; i < (ssize_t) argc; i++) argv[i]=DestroyString(argv[i]); argv=(char **) RelinquishMagickMemory(argv); }; return(MagickFalse); };
  if (image != (Image *) 
# 303 "MagickWand/conjure.c" 3 4
                        ((void *)0)
# 303 "MagickWand/conjure.c"
                            )
    image=DestroyImageList(image);
