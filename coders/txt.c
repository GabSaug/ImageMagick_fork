...
static MagickBooleanType IsTXT(const unsigned char *magick,const size_t length)
{
  ...
  if (length < 40)
    return(MagickFalse);
  if (LocaleNCompare((const char *) magick,"# ImageMagick pixel enumeration:",
        strlen("# ImageMagick pixel enumeration:")) != 0)
    return(MagickFalse);
...
# 194 "coders/txt.c" 3 4
 , "coders/txt.c", 194, __extension__ __PRETTY_FUNCTION__); }))
# 194 "coders/txt.c"
                                               ;
  
# 195 "coders/txt.c" 3 4
 ((void) sizeof ((
# 195 "coders/txt.c"
...
# 195 "coders/txt.c" 3 4
 , "coders/txt.c", 195, __extension__ __PRETTY_FUNCTION__); }))
# 195 "coders/txt.c"
                                                     ;
  if (IsEventLogging() != MagickFalse)
    (void) LogMagickEvent(TraceEvent,"coders/txt.c",__func__,(unsigned long) 197,"%s",
      image_info->filename);
...
# 232 "coders/txt.c"
  ...
  image->page.y=0;
  texture=(Image *) 
# 248 "coders/txt.c" 3 4
                   ((void *)0)
# 248 "coders/txt.c"
                       ;
  if (image_info->texture != (char *) 
# 249 "coders/txt.c" 3 4
                                     ((void *)0)
# 249 "coders/txt.c"
                                         )
    {
...
# 265 "coders/txt.c" 3 4
                                                 ((void *)0)
# 265 "coders/txt.c"
                                                     );
  (void) CloneString(&draw_info->text,image_info->filename);
  (void) FormatLocaleString(geometry,4096,"%gx%g%+g%+g",(double)
  ...
  if (status == MagickFalse)
    {
      ...
      { (void) ThrowMagickException(exception,"coders/txt.c",__func__,(unsigned long) 274,TypeError,"UnableToGetTypeMetrics", "`%s'",image_info->filename); if ((image) != (Image *) 
# 274 "coders/txt.c" 3 4
     ((void *)0)
# 274 "coders/txt.c"
...
# 274 "coders/txt.c"
  ...
  (void) FormatLocaleString(geometry,4096,"%gx%g%+g%+g",(double)
    image->columns,(double) image->rows,(double) page.x,(double) page.y);
  (void) CloneString(&draw_info->geometry,geometry);
  (void) CopyMagickString(filename,image_info->filename,4096);
  if (*draw_info->text != '\0')
    *draw_info->text='\0';
  p=text;
  for (offset=2*page.y; p != (char *) 
# 284 "coders/txt.c" 3 4
                                     ((void *)0)
# 284 "coders/txt.c"
                                         ; )
  {
...
# 292 "coders/txt.c" 3 4
                                    ((void *)0)
# 292 "coders/txt.c"
                                        )
      {
        status=SetImageProgress(image,"Load/Image",(MagickOffsetType) offset,
          image->rows);
        if (status == MagickFalse)
          break;
      }
    p=ReadBlobString(image,text);
    if ((offset < (ssize_t) image->rows) && (p != (char *) 
# 300 "coders/txt.c" 3 4
                                                          ((void *)0)
# 300 "coders/txt.c"
                                                              ))
      continue;
    if (texture != (Image *) 
# 302 "coders/txt.c" 3 4
                            ((void *)0)
# 302 "coders/txt.c"
                                )
      {
...
# 401 "coders/txt.c"
 "image_info != (const ImageInfo *) NULL"
# 401 "coders/txt.c" 3 4
 , "coders/txt.c", 401, __extension__ __PRETTY_FUNCTION__); }))
# 401 "coders/txt.c"
                                               ;
  
# 402 "coders/txt.c" 3 4
 ((void) sizeof ((
# 402 "coders/txt.c"
 image_info->signature == 0xabacadabUL
# 402 "coders/txt.c" 3 4
...
# 402 "coders/txt.c"
 "image_info->signature == MagickCoreSignature"
# 402 "coders/txt.c" 3 4
 , "coders/txt.c", 402, __extension__ __PRETTY_FUNCTION__); }))
# 402 "coders/txt.c"
                                                     ;
  
# 403 "coders/txt.c" 3 4
 ((void) sizeof ((
# 403 "coders/txt.c"
...
# 403 "coders/txt.c" 3 4
 , "coders/txt.c", 403, __extension__ __PRETTY_FUNCTION__); }))
# 403 "coders/txt.c"
                                            ;
  
# 404 "coders/txt.c" 3 4
 ((void) sizeof ((
# 404 "coders/txt.c"
...
# 404 "coders/txt.c" 3 4
 , "coders/txt.c", 404, __extension__ __PRETTY_FUNCTION__); }))
# 404 "coders/txt.c"
                                                    ;
  if (IsEventLogging() != MagickFalse)
    (void) LogMagickEvent(TraceEvent,"coders/txt.c",__func__,(unsigned long) 406,"%s",
      image_info->filename);
  image=AcquireImage(image_info,exception);
  status=OpenBlob(image_info,image,ReadBinaryBlobMode,exception);
  if (status == MagickFalse)
    {
      image=DestroyImageList(image);
      return((Image *) 
# 413 "coders/txt.c" 3 4
                      ((void *)0)
# 413 "coders/txt.c"
                          );
    }
  (void) ReadBlobString(image,text);
  if (LocaleNCompare((char *) text,"# ImageMagick pixel enumeration:",strlen("# ImageMagick pixel enumeration:")) != 0)
    { (void) ThrowMagickException(exception,"coders/txt.c",__func__,(unsigned long) 417,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 417 "coders/txt.c" 3 4
   ((void *)0)
# 417 "coders/txt.c"
   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 417 "coders/txt.c" 3 4
...
# 417 "coders/txt.c"
  ...
  do
  {
    ...
    if ((count < 4) || (width == 0) || (height == 0) || (max_value == 0.0) ||
        (number_meta_channels >= (unsigned long) (64 -MetaPixelChannels)))
      { (void) ThrowMagickException(exception,"coders/txt.c",__func__,(unsigned long) 450,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 450 "coders/txt.c" 3 4
     ((void *)0)
# 450 "coders/txt.c"
     ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 450 "coders/txt.c" 3 4
...
# 450 "coders/txt.c"
    ...
    if ((max_value == 0.0) || (max_value >= 18446744073709551615.0))
      { (void) ThrowMagickException(exception,"coders/txt.c",__func__,(unsigned long) 455,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 455 "coders/txt.c" 3 4
     ((void *)0)
# 455 "coders/txt.c"
     ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 455 "coders/txt.c" 3 4
...
# 455 "coders/txt.c"
    ...
    if (type < 0)
      { (void) ThrowMagickException(exception,"coders/txt.c",__func__,(unsigned long) 473,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 473 "coders/txt.c" 3 4
     ((void *)0)
# 473 "coders/txt.c"
     ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 473 "coders/txt.c" 3 4
...
# 473 "coders/txt.c"
    ...
    for (y=0; y < (ssize_t) image->rows; y++)
    {
      ...
      for (x=0; x < (ssize_t) image->columns; x++)
      {
        ...
        if (ReadBlobString(image,text) == (char *) 
# 506 "coders/txt.c" 3 4
                                                  ((void *)0)
# 506 "coders/txt.c"
                                                      )
          {
...
# 557 "coders/txt.c"
  ...
  } while (LocaleNCompare((char *) text,"# ImageMagick pixel enumeration:",strlen("# ImageMagick pixel enumeration:")) == 0);
...
# 598 "coders/txt.c"
__attribute__ ((visibility ("default"))) size_t RegisterTXTImage(void)
{
  MagickInfo
    *entry;
...
# 641 "coders/txt.c"
__attribute__ ((visibility ("default"))) void UnregisterTXTImage(void)
{
...
# 690 "coders/txt.c"
 "image_info != (const ImageInfo *) NULL"
# 690 "coders/txt.c" 3 4
 , "coders/txt.c", 690, __extension__ __PRETTY_FUNCTION__); }))
# 690 "coders/txt.c"
                                               ;
  
# 691 "coders/txt.c" 3 4
 ((void) sizeof ((
# 691 "coders/txt.c"
 image_info->signature == 0xabacadabUL
# 691 "coders/txt.c" 3 4
...
# 691 "coders/txt.c"
 "image_info->signature == MagickCoreSignature"
# 691 "coders/txt.c" 3 4
 , "coders/txt.c", 691, __extension__ __PRETTY_FUNCTION__); }))
# 691 "coders/txt.c"
                                                     ;
  
# 692 "coders/txt.c" 3 4
 ((void) sizeof ((
# 692 "coders/txt.c"
 image != (Image *) 
# 692 "coders/txt.c" 3 4
...
# 692 "coders/txt.c"
 "image != (Image *) NULL"
# 692 "coders/txt.c" 3 4
 , "coders/txt.c", 692, __extension__ __PRETTY_FUNCTION__); }))
# 692 "coders/txt.c"
                                ;
  
# 693 "coders/txt.c" 3 4
 ((void) sizeof ((
# 693 "coders/txt.c"
...
# 693 "coders/txt.c" 3 4
 , "coders/txt.c", 693, __extension__ __PRETTY_FUNCTION__); }))
# 693 "coders/txt.c"
                                                ;
  if (IsEventLogging() != MagickFalse)
    (void) LogMagickEvent(TraceEvent,"coders/txt.c",__func__,(unsigned long) 695,"%s",image->filename);
  ...
  do
  {
    ...
    (void) CopyMagickString(colorspace,CommandOptionToMnemonic(
      MagickColorspaceOptions,(ssize_t) image->colorspace),4096);
    LocaleLower(colorspace);
    image->depth=GetImageQuantumDepth(image,MagickTrue);
    range=GetQuantumRange(image->depth);
    if (image->alpha_trait != UndefinedPixelTrait)
      (void) ConcatenateMagickString(colorspace,"a",4096);
...
# 742 "coders/txt.c"
      ...
      for (x=0; x < (ssize_t) image->columns; x++)
      {
        ...
        if (LocaleCompare(image_info->magick,"SPARSE-COLOR") == 0)
          {
            ...
            if (GetPixelAlpha(image,p) == (Quantum) ((Quantum) ((Quantum) 65535.0)))
              {
                (void) FormatLocaleString(buffer,4096,
                  "%.20g,%.20g,",(double) x,(double) y);
                GetColorTuple(&pixel,MagickFalse,tuple);
                (void) ConcatenateMagickString(buffer,tuple,4096);
                (void) ConcatenateMagickString(buffer," ",4096);
                (void) WriteBlobString(image,buffer);
              }
        ...
        for (i=0; i < (ssize_t) GetImageChannels(image); i++)
        {
          ...
          (void) FormatLocaleString(tuple,4096,"%.20g",
            (double) channel);
          (void) ConcatenateMagickString(buffer,tuple,4096);
