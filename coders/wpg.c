          ...
          if(RunCount)
            {
              ...
              for(i=0;i<(int) RunCount;i++) { BImgBuff[x]=bbuf; x++; if((ssize_t) x>=ldblk) { if (InsertRow(image,BImgBuff,(ssize_t) y,bpp,exception) != MagickFalse) y++; x=0; } };
            }
          else {
            c=ReadBlobByte(image);
            if (c < 0)
            ...
            for(i=0;i<(int) RunCount;i++) { BImgBuff[x]=0xFF; x++; if((ssize_t) x>=ldblk) { if (InsertRow(image,BImgBuff,(ssize_t) y,bpp,exception) != MagickFalse) y++; x=0; } };
          }
        }
      else {
        if(RunCount)
            ...
            for(i=0;i < (int) RunCount;i++)
              {
                ...
                if (c < 0)
                  break;
                { BImgBuff[x]=c; x++; if((ssize_t) x>=ldblk) { if (InsertRow(image,BImgBuff,(ssize_t) y,bpp,exception) != MagickFalse) y++; x=0; } };
              }
          }
        else {
          c=ReadBlobByte(image);
          if (c == 
# 511 "coders/wpg.c" 3 4
                  (-1)
# 511 "coders/wpg.c"
                     )
            {
...
# 593 "coders/wpg.c"
  ...
  while( y< (ssize_t) image->rows)
  {
      ...
      switch(bbuf)
        {
        case 0x7D:
          SampleSize=(unsigned int) ReadBlobByte(image);
          if(SampleSize>8)
            {
              BImgBuff=(unsigned char *) RelinquishMagickMemory(BImgBuff);
              return(-2);
...
# 618 "coders/wpg.c"
        ...
        case 0x7F:
          ...
          for(i=0; i < ((ssize_t) SampleSize*(RunCount+1)); i++)
            {
              { if(XorMe) BImgBuff[x] = (unsigned char)~0; else BImgBuff[x] = 0; x++; if((ssize_t) x >= ldblk) { if (InsertRow(image,BImgBuff,(ssize_t) y,bpp,exception) != MagickFalse) y++; x=0; } };
            }
          break;
        case 0xFD:
          RunCount=ReadBlobByte(image);
          ...
          for(i=0; i<= RunCount;i++)
            for(bbuf=0; bbuf < SampleSize; bbuf++)
              { if(XorMe) BImgBuff[x] = (unsigned char)~SampleBuffer[bbuf]; else BImgBuff[x] = SampleBuffer[bbuf]; x++; if((ssize_t) x >= ldblk) { if (InsertRow(image,BImgBuff,(ssize_t) y,bpp,exception) != MagickFalse) y++; x=0; } };
          break;
        case 0xFE:
          RunCount=ReadBlobByte(image);
          if (RunCount < 0)
...
# 645 "coders/wpg.c"
        ...
        case 0xFF:
          ...
          for(i=0; i < ((ssize_t) SampleSize*(RunCount+1)); i++)
            {
              { if(XorMe) BImgBuff[x] = (unsigned char)~0xFF; else BImgBuff[x] = 0xFF; x++; if((ssize_t) x >= ldblk) { if (InsertRow(image,BImgBuff,(ssize_t) y,bpp,exception) != MagickFalse) y++; x=0; } };
            }
          break;
        default:
          RunCount=bbuf & 0x7F;
          ...
          if(bbuf & 0x80)
            ...
            for(i=0; i < (ssize_t) ((int) SampleSize*((int) RunCount+1)); i++)
              {
                bbuf=ReadBlobByte(image);
                { if(XorMe) BImgBuff[x] = (unsigned char)~bbuf; else BImgBuff[x] = bbuf; x++; if((ssize_t) x >= ldblk) { if (InsertRow(image,BImgBuff,(ssize_t) y,bpp,exception) != MagickFalse) y++; x=0; } };
              }
          }
        }
      if (EOFBlob(image) != MagickFalse)
  ...
  MagickOffsetType PS_Offset,ssize_t PS_Size,ExceptionInfo *exception)
{
  ...
  if ((clone_info=CloneImageInfo(image_info)) == 
# 801 "coders/wpg.c" 3 4
                                                ((void *)0)
# 801 "coders/wpg.c"
                                                    )
    return(image);
  clone_info->blob=(void *) 
# 803 "coders/wpg.c" 3 4
                           ((void *)0)
# 803 "coders/wpg.c"
                               ;
  clone_info->length=0;
...
# 810 "coders/wpg.c"
  ...
  if (SeekBlob(image,PS_Offset,
# 814 "coders/wpg.c" 3 4
                              0
# 814 "coders/wpg.c"
                                      ) != PS_Offset)
    {
  ...
  count=ReadBlob(image, 2*4096, magick);
  if (count < 1)
    {
      (void) fclose(ps_file);
      ThrowException(exception,CorruptImageError,"ImproperImageHeader",
...
# 853 "coders/wpg.c"
  ...
  FormatLocaleString(clone_info->filename,4096,"ps:%.1024s",
    postscript_file);
...

  {
    ...
    do
    {
      (void) CopyMagickString(p->filename,image->filename,4096);
      (void) CopyMagickString(p->magick_filename,image->magick_filename,
        4096);
      (void) CopyMagickString(p->magick,image->magick,4096);
      if ((p->rows == 0) || (p->columns == 0))
        {
          DeleteImageFromList(&p);
          if (p == (Image *) 
# 895 "coders/wpg.c" 3 4
                            ((void *)0)
# 895 "coders/wpg.c"
                                )
            {
              image2=(Image *) 
# 897 "coders/wpg.c" 3 4
                              ((void *)0)
# 897 "coders/wpg.c"
                                  ;
              goto FINISH_UNL;
...
# 1088 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1088, __extension__ __PRETTY_FUNCTION__); }))
# 1088 "coders/wpg.c"
                                               ;
  
# 1089 "coders/wpg.c" 3 4
 ((void) sizeof ((
# 1089 "coders/wpg.c"
...
# 1089 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1089, __extension__ __PRETTY_FUNCTION__); }))
# 1089 "coders/wpg.c"
                                                     ;
  
# 1090 "coders/wpg.c" 3 4
 ((void) sizeof ((
# 1090 "coders/wpg.c"
...
# 1090 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1090, __extension__ __PRETTY_FUNCTION__); }))
# 1090 "coders/wpg.c"
                                            ;
  
# 1091 "coders/wpg.c" 3 4
 ((void) sizeof ((
# 1091 "coders/wpg.c"
...
# 1091 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1091, __extension__ __PRETTY_FUNCTION__); }))
# 1091 "coders/wpg.c"
                                                    ;
  one=1;
...
# 1101 "coders/wpg.c"
  ...
  if ((Header.FileId != 0x435057FF) || (Header.FileType != 0x16))
    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1116,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1116 "coders/wpg.c" 3 4
   ((void *)0)
# 1116 "coders/wpg.c"
...
# 1116 "coders/wpg.c"
  ...
  if (Header.EncryptKey!=0)
    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1118,CoderError,"EncryptedWPGImageFileNotSupported", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1118 "coders/wpg.c" 3 4
   ((void *)0)
# 1118 "coders/wpg.c"
   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1118 "coders/wpg.c" 3 4
...
# 1118 "coders/wpg.c"
  ...
  switch(Header.MajorVersion)
  {
    case 1:
      while(!EOFBlob(image))
        {
          if (SeekBlob(image,Header.DataOffset,
# 1130 "coders/wpg.c" 3 4
                                              0
# 1130 "coders/wpg.c"
                                                      ) != Header.DataOffset)
            break;
...
# 1135 "coders/wpg.c"
          ...
          if ((Rec.RecordLength+4) >= GetBlobSize(image))
            { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1139,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1139 "coders/wpg.c" 3 4
           ((void *)0)
# 1139 "coders/wpg.c"
           ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1139 "coders/wpg.c" 3 4
...
# 1139 "coders/wpg.c"
          ...
          if (Header.DataOffset > (MagickOffsetType) GetBlobSize(image))
            { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1145,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1145 "coders/wpg.c" 3 4
           ((void *)0)
# 1145 "coders/wpg.c"
           ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1145 "coders/wpg.c" 3 4
          ...
          switch(Rec.RecType)
            ...
            case 0x0B:
              ...
              if ((BitmapHeader1.Width == 0) || (BitmapHeader1.Height == 0))
                { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1153,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1153 "coders/wpg.c" 3 4
               ((void *)0)
# 1153 "coders/wpg.c"
               ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1153 "coders/wpg.c" 3 4
...
# 1153 "coders/wpg.c"
            ...
            case 0x0E:
              ...
              if ((WPG_Palette.NumOfEntries-WPG_Palette.StartIndex) >
                ...
                { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1177,CorruptImageError,"InvalidColormapIndex", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1177 "coders/wpg.c" 3 4
               ((void *)0)
# 1177 "coders/wpg.c"
               ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1177 "coders/wpg.c" 3 4
...
# 1177 "coders/wpg.c"
              ...
              if (WPG_Palette.StartIndex > WPG_Palette.NumOfEntries)
                { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1179,CorruptImageError,"InvalidColormapIndex", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1179 "coders/wpg.c" 3 4
               ((void *)0)
# 1179 "coders/wpg.c"
               ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1179 "coders/wpg.c" 3 4
              ...
              if (Rec.RecordLength > 8)
                {
                  ...
                  if (image == 
# 1201 "coders/wpg.c" 3 4
                              ((void *)0)
# 1201 "coders/wpg.c"
                                  )
                    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1202,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1202 "coders/wpg.c" 3 4
                   ((void *)0)
# 1202 "coders/wpg.c"
                   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1202 "coders/wpg.c" 3 4
...
# 1202 "coders/wpg.c"
            ...
            case 0x14:
              ...
              if ((BitmapHeader2.Width == 0) || (BitmapHeader2.Height == 0))
                { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1216,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1216 "coders/wpg.c" 3 4
               ((void *)0)
# 1216 "coders/wpg.c"
               ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1216 "coders/wpg.c" 3 4
              ...
              if ((image->storage_class != PseudoClass) && (bpp < 24))
                {
                  ...
                  if (image->colors > GetBlobSize(image))
                    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1246,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1246 "coders/wpg.c" 3 4
                   ((void *)0)
# 1246 "coders/wpg.c"
                   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1246 "coders/wpg.c" 3 4
...
# 1246 "coders/wpg.c"
                  ...
                  if (!AcquireImageColormap(image,image->colors,exception))
                    {
                    NoMemory:
                      { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1251,ResourceLimitError,"MemoryAllocationFailed", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1251 "coders/wpg.c" 3 4
                     ((void *)0)
# 1251 "coders/wpg.c"
                     ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1251 "coders/wpg.c" 3 4
                  ...
                  for (i=0; (i < (int) image->colors) && (i < 256); i++)
                    {
                      ...
                      image->colormap[i].alpha=((Quantum) ((Quantum) 65535.0));
                    }
                }
              else
                {
...
# 1275 "coders/wpg.c"
                      ...
                      (void) memcpy(image->colormap,colormap,(((image->colors) < (colors)) ? (image->colors) : (colors))
              ...
              if (bpp == 1)
                {
                  ...
                  image->colormap[0].alpha=((Quantum) ((Quantum) 65535.0));
                  image->colormap[1].red=image->colormap[1].green=
                    image->colormap[1].blue=((Quantum) 65535.0);
                  image->colormap[1].alpha=((Quantum) ((Quantum) 65535.0));
                }
              if(!image_info->ping)
                if(UnpackWPGRaster(image,bpp,exception) < 0)
                  ...
                  {
                  DecompressionFailed:
                    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1303,CoderError,"UnableToDecompressImage", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1303 "coders/wpg.c" 3 4
                   ((void *)0)
# 1303 "coders/wpg.c"
                   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1303 "coders/wpg.c" 3 4
              ...
              if (Rec.RecordLength>0x3C)
                {
                  ...
                  if (image == 
# 1367 "coders/wpg.c" 3 4
                              ((void *)0)
# 1367 "coders/wpg.c"
                                  )
                    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1368,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1368 "coders/wpg.c" 3 4
                   ((void *)0)
# 1368 "coders/wpg.c"
                   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1368 "coders/wpg.c" 3 4
...
# 1390 "coders/wpg.c"
      ...
      case 1:
            ...
            case 0x0C:
              ...
              if ((WPG_Palette.NumOfEntries-WPG_Palette.StartIndex) >
                  (Rec2.RecordLength-2-2) / 3)
                { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1412,CorruptImageError,"InvalidColormapIndex", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1412 "coders/wpg.c" 3 4
               ((void *)0)
# 1412 "coders/wpg.c"
               ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1412 "coders/wpg.c" 3 4
...
# 1412 "coders/wpg.c"
              ...
              if (WPG_Palette.StartIndex >= WPG_Palette.NumOfEntries)
                { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1414,CorruptImageError,"InvalidColormapIndex", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1414 "coders/wpg.c" 3 4
               ((void *)0)
# 1414 "coders/wpg.c"
               ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1414 "coders/wpg.c" 3 4
...
# 1414 "coders/wpg.c"
              ...
              image->colors=WPG_Palette.NumOfEntries;
              if (AcquireImageColormap(image,image->colors,exception) == MagickFalse)
                { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1417,ResourceLimitError,"MemoryAllocationFailed", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1417 "coders/wpg.c" 3 4
               ((void *)0)
# 1417 "coders/wpg.c"
               ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1417 "coders/wpg.c" 3 4
...
# 1417 "coders/wpg.c"
            ...
            case 0x0E:
              ...
              if ((Bitmap2Header1.Width == 0) || (Bitmap2Header1.Height == 0))
                { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1436,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1436 "coders/wpg.c" 3 4
               ((void *)0)
# 1436 "coders/wpg.c"
               ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1436 "coders/wpg.c" 3 4
              ...
              if (Rec2.RecordLength > (unsigned int) i)
                {
                  ...
                  if (image == 
# 1576 "coders/wpg.c" 3 4
                              ((void *)0)
# 1576 "coders/wpg.c"
                                  )
                    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1577,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1577 "coders/wpg.c" 3 4
                   ((void *)0)
# 1577 "coders/wpg.c"
                   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1577 "coders/wpg.c" 3 4
...
# 1577 "coders/wpg.c"
      ...
      case 0x1B:
              WPG2Flags = (int) LoadWPG2Flags(image,(char)
                StartWPG.PosSizePrecision,
# 1584 "coders/wpg.c" 3 4
                                         ((void *)0)
# 1584 "coders/wpg.c"
                                             ,&CTM);
              (void) WPG2Flags;
    ...
    default:
      {
         { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1594,CoderError,"DataEncodingSchemeIsNotSupported", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1594 "coders/wpg.c" 3 4
        ((void *)0)
# 1594 "coders/wpg.c"
        ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1594 "coders/wpg.c" 3 4
...
# 1594 "coders/wpg.c"
 ...
 Finish:
...

  {
    ...
    p=image;
    image=
# 1612 "coders/wpg.c" 3 4
         ((void *)0)
# 1612 "coders/wpg.c"
             ;
    while (p != (Image *) 
# 1613 "coders/wpg.c" 3 4
                         ((void *)0)
# 1613 "coders/wpg.c"
                             )
      {
...
# 1627 "coders/wpg.c"
  ...
  if (image == (Image *) 
# 1630 "coders/wpg.c" 3 4
                        ((void *)0)
# 1630 "coders/wpg.c"
                            )
    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1631,CorruptImageError,"ImageFileDoesNotContainAnyImageData", "`%s'",image_info->filename); if ((image) != (Image *) 
# 1631 "coders/wpg.c" 3 4
   ((void *)0)
# 1631 "coders/wpg.c"
   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 1631 "coders/wpg.c" 3 4
...
# 1659 "coders/wpg.c"
__attribute__ ((visibility ("default"))) size_t RegisterWPGImage(void)
{
  MagickInfo
    *entry;
...
# 1693 "coders/wpg.c"
__attribute__ ((visibility ("default"))) void UnregisterWPGImage(void)
{
...
# 1854 "coders/wpg.c"
 "image_info != (const ImageInfo *) NULL"
# 1854 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1854, __extension__ __PRETTY_FUNCTION__); }))
# 1854 "coders/wpg.c"
                                               ;
  
# 1855 "coders/wpg.c" 3 4
 ((void) sizeof ((
# 1855 "coders/wpg.c"
 image_info->signature == 0xabacadabUL
# 1855 "coders/wpg.c" 3 4
...
# 1855 "coders/wpg.c"
 "image_info->signature == MagickCoreSignature"
# 1855 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1855, __extension__ __PRETTY_FUNCTION__); }))
# 1855 "coders/wpg.c"
                                                     ;
  
# 1856 "coders/wpg.c" 3 4
 ((void) sizeof ((
# 1856 "coders/wpg.c"
 image != (Image *) 
# 1856 "coders/wpg.c" 3 4
...
# 1856 "coders/wpg.c"
 "image != (Image *) NULL"
# 1856 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1856, __extension__ __PRETTY_FUNCTION__); }))
# 1856 "coders/wpg.c"
                                ;
  
# 1857 "coders/wpg.c" 3 4
 ((void) sizeof ((
# 1857 "coders/wpg.c"
...
# 1857 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1857, __extension__ __PRETTY_FUNCTION__); }))
# 1857 "coders/wpg.c"
                                                ;
  
# 1858 "coders/wpg.c" 3 4
 ((void) sizeof ((
# 1858 "coders/wpg.c"
 exception != (ExceptionInfo *) 
# 1858 "coders/wpg.c" 3 4
...
# 1858 "coders/wpg.c"
 "exception != (ExceptionInfo *) NULL"
# 1858 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1858, __extension__ __PRETTY_FUNCTION__); }))
# 1858 "coders/wpg.c"
                                            ;
  
# 1859 "coders/wpg.c" 3 4
 ((void) sizeof ((
# 1859 "coders/wpg.c"
 exception->signature == 0xabacadabUL
# 1859 "coders/wpg.c" 3 4
...
# 1859 "coders/wpg.c"
 "exception->signature == MagickCoreSignature"
# 1859 "coders/wpg.c" 3 4
 , "coders/wpg.c", 1859, __extension__ __PRETTY_FUNCTION__); }))
# 1859 "coders/wpg.c"
                                                    ;
  if (IsEventLogging() != MagickFalse)
    (void) LogMagickEvent(TraceEvent,"coders/wpg.c",__func__,(unsigned long) 1861,"%s",image->filename);
  status=OpenBlob(image_info,image,WriteBinaryBlobMode,exception);
  if (status == MagickFalse)
    return(status);
  if ((image->columns > 65535UL) || (image->rows > 65535UL))
    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1866,ImageError,"WidthOrHeightExceedsLimit", "`%s'",image->filename); if (image_info->adjoin != MagickFalse) while (image->previous != (Image *) 
# 1866 "coders/wpg.c" 3 4
   ((void *)0)
# 1866 "coders/wpg.c"
  ...
  (void) WriteBlobLSBLong(image,0x435057FF);
  (void) WriteBlobLSBLong(image,16);
  (void) WriteBlobByte(image,1);
  (void) WriteBlobByte(image,0x16);
  (void) WriteBlobByte(image,1);
...
# 1947 "coders/wpg.c" 3 4
                                     ((void *)0)
# 1947 "coders/wpg.c"
                                         )
    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1948,ImageError,"MemoryAllocationFailed", "`%s'",image->filename); if (image_info->adjoin != MagickFalse) while (image->previous != (Image *) 
# 1948 "coders/wpg.c" 3 4
   ((void *)0)
# 1948 "coders/wpg.c"
   ) image=image->previous; (void) CloseBlob(image); return(MagickFalse); };
  ...
  for (y=0; y < (ssize_t) image->rows; y++)
  {
    ...
    if (p == (const Quantum *) 
# 1963 "coders/wpg.c" 3 4
                              ((void *)0)
# 1963 "coders/wpg.c"
                                  )
      break;
    length=ExportQuantumPixels(image,(CacheView *) 
# 1965 "coders/wpg.c" 3 4
                                                  ((void *)0)
# 1965 "coders/wpg.c"
                                                      ,quantum_info,
...
# 1980 "coders/wpg.c"
  ...
  if (y < (ssize_t) image->rows)
    { (void) ThrowMagickException(exception,"coders/wpg.c",__func__,(unsigned long) 1985,CorruptImageError,"UnableToWriteImageData", "`%s'",image->filename); if (image_info->adjoin != MagickFalse) while (image->previous != (Image *) 
# 1985 "coders/wpg.c" 3 4
   ((void *)0)
# 1985 "coders/wpg.c"
   ) image=image->previous; (void) CloseBlob(image); return(MagickFalse); };
