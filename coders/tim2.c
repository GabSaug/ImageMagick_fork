...
static inline Quantum GetAlpha(unsigned int word,TIM2ColorEncoding ce)
{
  switch(ce)
  {
    ...
    case RGBA32:
      ...
      return ScaleCharToQuantum(((((word>>3*8&0xFF)<<1) < (0xFF)) ? ((word>>3*8&0xFF)<<1) : (0xFF)));
    default:
      return 0xFF;
  }
}
  ...
  ExceptionInfo *exception)
{
  ...
  if (row_data == (unsigned char *) 
# 275 "coders/tim2.c" 3 4
                                   ((void *)0)
# 275 "coders/tim2.c"
                                       )
    { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 276,ResourceLimitError,"MemoryAllocationFailed", "`%s'",image_info->filename); return(MagickFalse); }
                           ;
  if (clut_depth != 0)
    {
      ...
      if (AcquireImageColormap(image,image->colors,exception) == MagickFalse)
        {
          ...
          { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 284,ResourceLimitError,"MemoryAllocationFailed", "`%s'",image_info->filename); return(MagickFalse); }
                                 ;
        }
...
# 294 "coders/tim2.c"
            ...
            if (count != (ssize_t) bytes_per_line)
              {
                row_data=(unsigned char *) RelinquishMagickMemory(row_data);
                { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 300,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); return(MagickFalse); }
                                                                     ;
              }
            p=row_data;
...
# 320 "coders/tim2.c" 3 4
                                            ((void *)0)
# 320 "coders/tim2.c"
                                                )
              {
                status=SetImageProgress(image,"Load/Image",
...
# 335 "coders/tim2.c"
            ...
            if (count != (ssize_t) bytes_per_line)
              {
                row_data=(unsigned char *) RelinquishMagickMemory(row_data);
                { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 341,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); return(MagickFalse); }
                                                                     ;
              }
            p=row_data;
...
# 353 "coders/tim2.c"
        ...
        {
          row_data=(unsigned char *) RelinquishMagickMemory(row_data);
          { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 366,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); return(MagickFalse); }
                                 ;
        }
      }
...
# 381 "coders/tim2.c"
            ...
            if (count != (ssize_t) bytes_per_line)
              {
                row_data=(unsigned char *) RelinquishMagickMemory(row_data);
                { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 387,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); return(MagickFalse); }
                                                                     ;
              }
            p=row_data;
            for (x=0; x < (ssize_t) image->columns; x++)
...
# 420 "coders/tim2.c"
            ...
            if (count != (ssize_t) bytes_per_line)
              {
                row_data=(unsigned char *) RelinquishMagickMemory(row_data);
                { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 426,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); return(MagickFalse); }
                                                                     ;
              }
            p=row_data;
            for (x=0; x < (ssize_t) image->columns; x++)
...
# 459 "coders/tim2.c"
            ...
            if (count != (ssize_t) bytes_per_line)
              {
                row_data=(unsigned char *) RelinquishMagickMemory(row_data);
                { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 465,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); return(MagickFalse); }
                                                                     ;
              }
            p=row_data;
            for (x=0; x < (ssize_t) image->columns; x++)
...
# 485 "coders/tim2.c"
        ...
        {
          row_data=(unsigned char *) RelinquishMagickMemory(row_data);
          { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 498,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); return(MagickFalse); }
                                 ;
        }
      }
    }
  ...
  if ((status != MagickFalse) && (clut_depth != 0))
  {
    ...
    clut_size=(((header->clut_size) > ((size_t) (clut_depth/8)* image->colors)) ? (header->clut_size) : ((size_t) (clut_depth/8)* image->colors))
                    ;
    clut_data=(unsigned char *) AcquireQuantumMemory(clut_size,
      sizeof(*clut_data));
    if (clut_data == (unsigned char *) 
# 525 "coders/tim2.c" 3 4
                                      ((void *)0)
# 525 "coders/tim2.c"
                                          )
      { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 526,ResourceLimitError,"MemoryAllocationFailed", "`%s'",image_info->filename); return(MagickFalse); }
                             ;
    ...
    if (count != (ssize_t) clut_size)
      {
        ...
        { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 533,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); return(MagickFalse); }
                               ;
      }
    ...
    switch ((int) header->clut_type>>4)
    {
      ...
      case 1:
        ...
        break;
      default:
        { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 601,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); return(MagickFalse); }
                               ;
        break;
    }
    if (csm == CSM1)
      {
        ...
        if (oldColormap == (PixelInfo *) 
# 612 "coders/tim2.c" 3 4
                                        ((void *)0)
# 612 "coders/tim2.c"
                                            )
          { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 613,ResourceLimitError,"MemoryAllocationFailed", "`%s'",image_info->filename); return(MagickFalse); }
                                 ;
...
# 640 "coders/tim2.c" 3 4
 , "coders/tim2.c", 640, __extension__ __PRETTY_FUNCTION__); }))
# 640 "coders/tim2.c"
                                               ;
  
# 641 "coders/tim2.c" 3 4
 ((void) sizeof ((
# 641 "coders/tim2.c"
...
# 641 "coders/tim2.c" 3 4
 , "coders/tim2.c", 641, __extension__ __PRETTY_FUNCTION__); }))
# 641 "coders/tim2.c"
                                                     ;
  
# 642 "coders/tim2.c" 3 4
 ((void) sizeof ((
# 642 "coders/tim2.c"
...
# 642 "coders/tim2.c" 3 4
 , "coders/tim2.c", 642, __extension__ __PRETTY_FUNCTION__); }))
# 642 "coders/tim2.c"
                                            ;
  
# 643 "coders/tim2.c" 3 4
 ((void) sizeof ((
# 643 "coders/tim2.c"
...
# 643 "coders/tim2.c" 3 4
 , "coders/tim2.c", 643, __extension__ __PRETTY_FUNCTION__); }))
# 643 "coders/tim2.c"
                                                    ;
  if (IsEventLogging() != MagickFalse)
    (void) LogMagickEvent(TraceEvent,"coders/tim2.c",__func__,(unsigned long) 645,"%s",
      image_info->filename);
...
# 652 "coders/tim2.c"
  ...
  if (file_header.magic_num != 0x54494D32)
    { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 656,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 656 "coders/tim2.c" 3 4
   ((void *)0)
# 656 "coders/tim2.c"
...
# 656 "coders/tim2.c"
  ...
  if (file_header.format_vers != 0x04)
    { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 659,CoderError,"ImageTypeNotSupported", "`%s'",image_info->filename); if ((image) != (Image *) 
# 659 "coders/tim2.c" 3 4
   ((void *)0)
# 659 "coders/tim2.c"
   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 659 "coders/tim2.c" 3 4
...
# 659 "coders/tim2.c"
  ...
  if (DiscardBlobBytes(image,8) == MagickFalse)
    { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 663,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); if ((image) != (Image *) 
# 663 "coders/tim2.c" 3 4
   ((void *)0)
# 663 "coders/tim2.c"
   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 663 "coders/tim2.c" 3 4
...
# 663 "coders/tim2.c"
  ...
  if ((file_header.format_type > 0) &&
    ...
    { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 666,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); if ((image) != (Image *) 
# 666 "coders/tim2.c" 3 4
   ((void *)0)
# 666 "coders/tim2.c"
   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 666 "coders/tim2.c" 3 4
...
# 666 "coders/tim2.c"
  ...
  if (file_header.image_count != 1)
    { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 671,CoderError,"NumberOfImagesIsNotSupported", "`%s'",image_info->filename); if ((image) != (Image *) 
# 671 "coders/tim2.c" 3 4
   ((void *)0)
# 671 "coders/tim2.c"
   ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 671 "coders/tim2.c" 3 4
...
# 693 "coders/tim2.c"
        ...
        status=SetImageProgress(image,"Load/Images",(MagickOffsetType)
    ...
    if (image_header.mipmap_count != 1)
      { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 706,CoderError,"NumberOfImagesIsNotSupported", "`%s'",image_info->filename); if ((image) != (Image *) 
# 706 "coders/tim2.c" 3 4
     ((void *)0)
# 706 "coders/tim2.c"
     ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 706 "coders/tim2.c" 3 4
...
# 706 "coders/tim2.c"
    ...
    if (image_header.header_size < 48)
      { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 708,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 708 "coders/tim2.c" 3 4
     ((void *)0)
# 708 "coders/tim2.c"
     ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 708 "coders/tim2.c" 3 4
...
# 708 "coders/tim2.c"
    ...
    if ((MagickSizeType) image_header.image_size > GetBlobSize(image))
      { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 710,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); if ((image) != (Image *) 
# 710 "coders/tim2.c" 3 4
     ((void *)0)
# 710 "coders/tim2.c"
     ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 710 "coders/tim2.c" 3 4
...
# 710 "coders/tim2.c"
    ...
    if ((MagickSizeType) image_header.clut_size > GetBlobSize(image))
      { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 712,CorruptImageError,"InsufficientImageDataInFile", "`%s'",image_info->filename); if ((image) != (Image *) 
# 712 "coders/tim2.c" 3 4
     ((void *)0)
# 712 "coders/tim2.c"
     ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 712 "coders/tim2.c" 3 4
    ...
    if (image_header.clut_type !=0)
      {
        switch((int) image_header.clut_type&0x0F)
        {
          ...
          case 3:
            ...
            break;
          default:
            { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 730,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 730 "coders/tim2.c" 3 4
           ((void *)0)
# 730 "coders/tim2.c"
           ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 730 "coders/tim2.c" 3 4
...
# 730 "coders/tim2.c"
    ...
    switch ((int) image_header.bpp_type)
    {
      ...
      case 5:
        ...
        break;
      default:
        { (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 752,CorruptImageError,"ImproperImageHeader", "`%s'",image_info->filename); if ((image) != (Image *) 
# 752 "coders/tim2.c" 3 4
       ((void *)0)
# 752 "coders/tim2.c"
       ) { (void) CloseBlob(image); image=DestroyImageList(image); } return((Image *) 
# 752 "coders/tim2.c" 3 4
...
# 770 "coders/tim2.c" 3 4
       (*__errno_location ())
# 770 "coders/tim2.c"
       ); (void) ThrowMagickException(exception,"coders/tim2.c",__func__,(unsigned long) 770,CorruptImageError,"UnexpectedEndOfFile", "'%s': %s",image->filename,file_message); file_message=DestroyString(file_message); }
                          ;
        break;
      }
  }
...
# 805 "coders/tim2.c"
__attribute__ ((visibility ("default"))) size_t RegisterTIM2Image(void)
{
  MagickInfo
    *entry;
...
# 835 "coders/tim2.c"
__attribute__ ((visibility ("default"))) void UnregisterTIM2Image(void)
{
