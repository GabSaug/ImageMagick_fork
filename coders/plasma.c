...
# 67 "coders/plasma.c" 2
# 95 "coders/plasma.c"
static inline MagickBooleanType PlasmaPixel(Image *image,
  RandomInfo *__restrict__ random_info,const double x,const double y,
  ExceptionInfo *exception)
{
  ...
  q=GetAuthenticPixels(image,(ssize_t) (x+0.5),(ssize_t) (y+0.5),1,1,
    exception);
  if (q == (Quantum *) 
# 104 "coders/plasma.c" 3 4
                      ((void *)0)
# 104 "coders/plasma.c"
                          )
    return(MagickFalse);
  SetPixelRed(image,((double) ((Quantum) 65535.0)*GetPseudoRandomValue(random_info)+
    0.5),q);
  SetPixelGreen(image,((double) ((Quantum) 65535.0)*GetPseudoRandomValue(random_info)+
    0.5),q);
  SetPixelBlue(image,((double) ((Quantum) 65535.0)*GetPseudoRandomValue(random_info)+
    0.5),q);
...
# 150 "coders/plasma.c"
  ...
  if (image == (Image *) 
# 155 "coders/plasma.c" 3 4
                        ((void *)0)
# 155 "coders/plasma.c"
                            )
    return((Image *) 
# 156 "coders/plasma.c" 3 4
                    ((void *)0)
# 156 "coders/plasma.c"
                        );
...
# 163 "coders/plasma.c"
    ...
    {
      SetPixelAlpha(image,((Quantum) 65535.0)/2,q);
  ...
  i=(size_t) (((image->columns) > (image->rows)) ? (image->columns) : (image->rows))/2;
  ...
  for (depth=1; ; depth++)
  {
    if (PlasmaImage(image,&segment_info,0,depth,exception) != MagickFalse)
      break;
    status=SetImageProgress(image,"Load/Image",(MagickOffsetType) depth,
      max_depth);
    if (status == MagickFalse)
      break;
  }
...
# 246 "coders/plasma.c"
__attribute__ ((visibility ("default"))) size_t RegisterPLASMAImage(void)
{
  MagickInfo
    *entry;
...
# 283 "coders/plasma.c"
__attribute__ ((visibility ("default"))) void UnregisterPLASMAImage(void)
{
