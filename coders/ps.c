...
# 299 "coders/ps.c"
                                               ; c=ReadMagickByteBuffer(&buffer))
  {
    ...
    if (skip != MagickFalse)
      continue;
    if ((*version == '\0') &&
        (CompareMagickByteBuffer(&buffer,"PS-",strlen("PS-")) != MagickFalse))
      {
        i=0;
        for (c=ReadMagickByteBuffer(&buffer); c != 
# 343 "coders/ps.c" 3 4
...
# 343 "coders/ps.c"
    ...
    if (CompareMagickByteBuffer(&buffer,"DocumentProcessColors:",length) != MagickFalse)
      {
        p=GetMagickByteBufferDatum(&buffer);
        if ((StringLocateSubstring(p,"Cyan") != (char *) 
# 367 "coders/ps.c" 3 4
                                                        ((void *)0)
# 367 "coders/ps.c"
                                                            ) ||
            (StringLocateSubstring(p,"Magenta") != (char *) 
# 368 "coders/ps.c" 3 4
                                                           ((void *)0)
# 368 "coders/ps.c"
                                                               ) ||
            (StringLocateSubstring(p,"Yellow") != (char *) 
# 369 "coders/ps.c" 3 4
                                                          ((void *)0)
# 369 "coders/ps.c"
                                                              ))
    ...
    if (spot_color == MagickFalse)
      {
        length=strlen("+ ");
        if (CompareMagickByteBuffer(&buffer,"+ ",length) != MagickFalse)
          {
            spot_color=MagickTrue;
            SkipMagickByteBuffer(&buffer,length+1);
          }
    ...
    if (spot_color != MagickFalse)
      {
        ...
        i=0;
        for (c=PeekMagickByteBuffer(&buffer); c != 
# 414 "coders/ps.c" 3 4
                                                  (-1)
# 414 "coders/ps.c"
                                                     ; c=PeekMagickByteBuffer(&buffer))
        {
          if ((c == '\r') || (c == '\n') || ((i+1) == 4096))
            {
              new_line=MagickTrue;
    ...
    if ((ps_info->icc_profile == (StringInfo *) 
# 431 "coders/ps.c" 3 4
                                               ((void *)0)
# 431 "coders/ps.c"
                                                   ) &&
        (CompareMagickByteBuffer(&buffer,"BeginICCProfile:",strlen("BeginICCProfile:")) != MagickFalse))
      {
        unsigned char
          *datum;
        ...
        if (SkipMagickByteBufferUntilNewline(&buffer) != MagickFalse)
          {
            ps_info->icc_profile=AcquireStringInfo(4096);
            datum=GetStringInfoDatum(ps_info->icc_profile);
            for (i=0; (c=ProfileInteger(&buffer,hex_digits)) != 
# 444 "coders/ps.c" 3 4
                                                               (-1)
# 444 "coders/ps.c"
                                                                  ; i++)
    ...
    if ((ps_info->photoshop_profile == (StringInfo *) 
# 457 "coders/ps.c" 3 4
                                                     ((void *)0)
# 457 "coders/ps.c"
                                                         ) &&
        (CompareMagickByteBuffer(&buffer,"BeginPhotoshop:",strlen("BeginPhotoshop:")) != MagickFalse))
      {
        unsigned long
          extent;
        ...
        p=GetMagickByteBufferDatum(&buffer);
        extent=0;
        count=(ssize_t) sscanf(p,"BeginPhotoshop:" " %lu",&extent);
        if ((count != 1) || (extent == 0))
          continue;
...
# 617 "coders/ps.c" 3 4
 , "coders/ps.c", 617, __extension__ __PRETTY_FUNCTION__); }))
# 617 "coders/ps.c"
                                               ;
...
# 618 "coders/ps.c" 3 4
 , "coders/ps.c", 618, __extension__ __PRETTY_FUNCTION__); }))
# 618 "coders/ps.c"
                                                     ;
...
# 619 "coders/ps.c" 3 4
 , "coders/ps.c", 619, __extension__ __PRETTY_FUNCTION__); }))
# 619 "coders/ps.c"
                                            ;
...
# 620 "coders/ps.c" 3 4
 , "coders/ps.c", 620, __extension__ __PRETTY_FUNCTION__); }))
# 620 "coders/ps.c"
                                                    ;
  if (IsEventLogging() != MagickFalse)
    (void) LogMagickEvent(TraceEvent,"coders/ps.c",__func__,(unsigned long) 622,"%s",
      image_info->filename);
...
# 634 "coders/ps.c" 3 4
     (*__errno_location ())
# 634 "coders/ps.c"
     ); (void) ThrowMagickException(exception,"coders/ps.c",__func__,(unsigned long) 634,FileOpenError,"UnableToCreateTemporaryFile", "'%s': %s",image_info->filename,file_message); file_message=DestroyString(file_message); }
                             ;
      image=DestroyImageList(image);
      return((Image *) 
# 637 "coders/ps.c" 3 4
...
# 663 "coders/ps.c"
  ...
  if ((fabs(info.bounds.x2-info.bounds.x1) >= 1.0e-12) &&
      (fabs(info.bounds.y2-info.bounds.y1) >= 1.0e-12))
    {
...
# 691 "coders/ps.c"
                                  )
    {
      ...
      if (flags == NoValue)
        {
          (void) ThrowMagickException(exception,"coders/ps.c",__func__,(unsigned long) 701,OptionError,
...
# 731 "coders/ps.c" 3 4
     (*__errno_location ())
# 731 "coders/ps.c"
     ); (void) ThrowMagickException(exception,"coders/ps.c",__func__,(unsigned long) 731,FileOpenError,"UnableToOpenFile", "'%s': %s",image_info->filename,file_message); file_message=DestroyString(file_message); }
                             ;
      CleanupPSInfo(&info);
      image=DestroyImageList(image);
      return((Image *) 
...
# 741 "coders/ps.c"
    ...
    {
      char
        translate_geometry[4096];

      (void) FormatLocaleString(translate_geometry,4096,
...
# 768 "coders/ps.c"
  ...
  (void) FormatLocaleString(density,4096,"%gx%g",resolution.x,
    resolution.y);
  if (crop == MagickFalse)
    {
...
# 794 "coders/ps.c"
  ...
  if (*image_info->magick == 'E')
    {
      if (crop != MagickFalse)
        (void) ConcatenateMagickString(options,"-dEPSCrop ",4096);
  ...
  *message='\0';
  ...
  if ((status == MagickFalse) ||
    ...
    {
      (void) ConcatenateMagickString(command," -c showpage",4096);
      status=InvokeGhostscriptDelegate(read_info->verbose,command,message,
        exception);
    }
  (void) RelinquishUniqueFileResource(postscript_filename);
...
# 857 "coders/ps.c"
    ...
    {
      if (*message != '\0')
        (void) ThrowMagickException(exception,"coders/ps.c",__func__,(unsigned long) 860,
...
# 872 "coders/ps.c"
  ...
  if (info.icc_profile != (StringInfo *) 
# 878 "coders/ps.c" 3 4
                                        ((void *)0)
# 878 "coders/ps.c"
                                            )
...
# 880 "coders/ps.c"
  ...
  if (info.xmp_profile != (StringInfo *) 
# 882 "coders/ps.c" 3 4
                                        ((void *)0)
# 882 "coders/ps.c"
                                            )
...
# 923 "coders/ps.c"
  ...
  image=DestroyImageList(image);
  scene=0;
  for (next=GetFirstImageInList(postscript_image); next != (Image *) 
# 926 "coders/ps.c" 3 4
                                                                    ((void *)0)
# 926 "coders/ps.c"
                                                                        ; )
...
# 957 "coders/ps.c"
__attribute__ ((visibility ("default"))) size_t RegisterPSImage(void)
{
  MagickInfo
    *entry;
...
# 1030 "coders/ps.c"
__attribute__ ((visibility ("default"))) void UnregisterPSImage(void)
{
...
# 1456 "coders/ps.c"
 "image_info != (const ImageInfo *) NULL"
# 1456 "coders/ps.c" 3 4
 , "coders/ps.c", 1456, __extension__ __PRETTY_FUNCTION__); }))
# 1456 "coders/ps.c"
                                               ;
  
# 1457 "coders/ps.c" 3 4
 ((void) sizeof ((
# 1457 "coders/ps.c"
 image_info->signature == 0xabacadabUL
# 1457 "coders/ps.c" 3 4
...
# 1457 "coders/ps.c"
 "image_info->signature == MagickCoreSignature"
# 1457 "coders/ps.c" 3 4
 , "coders/ps.c", 1457, __extension__ __PRETTY_FUNCTION__); }))
# 1457 "coders/ps.c"
                                                     ;
  
# 1458 "coders/ps.c" 3 4
 ((void) sizeof ((
# 1458 "coders/ps.c"
 image != (Image *) 
# 1458 "coders/ps.c" 3 4
...
# 1458 "coders/ps.c"
 "image != (Image *) NULL"
# 1458 "coders/ps.c" 3 4
 , "coders/ps.c", 1458, __extension__ __PRETTY_FUNCTION__); }))
# 1458 "coders/ps.c"
                                ;
  
# 1459 "coders/ps.c" 3 4
 ((void) sizeof ((
# 1459 "coders/ps.c"
...
# 1459 "coders/ps.c" 3 4
 , "coders/ps.c", 1459, __extension__ __PRETTY_FUNCTION__); }))
# 1459 "coders/ps.c"
                                                ;
  
# 1460 "coders/ps.c" 3 4
 ((void) sizeof ((
# 1460 "coders/ps.c"
 exception != (ExceptionInfo *) 
# 1460 "coders/ps.c" 3 4
...
# 1460 "coders/ps.c"
 "exception != (ExceptionInfo *) NULL"
# 1460 "coders/ps.c" 3 4
 , "coders/ps.c", 1460, __extension__ __PRETTY_FUNCTION__); }))
# 1460 "coders/ps.c"
                                            ;
  
# 1461 "coders/ps.c" 3 4
 ((void) sizeof ((
# 1461 "coders/ps.c"
 exception->signature == 0xabacadabUL
# 1461 "coders/ps.c" 3 4
...
# 1461 "coders/ps.c"
 "exception->signature == MagickCoreSignature"
# 1461 "coders/ps.c" 3 4
 , "coders/ps.c", 1461, __extension__ __PRETTY_FUNCTION__); }))
# 1461 "coders/ps.c"
                                                    ;
  if (IsEventLogging() != MagickFalse)
    (void) LogMagickEvent(TraceEvent,"coders/ps.c",__func__,(unsigned long) 1463,"%s",image->filename);
...
# 1588 "coders/ps.c"
                                         )
          {
            ...
            for (i=0; i < (ssize_t) GetStringInfoLength(profile); i++)
            {
              ...
              (void) FormatLocaleString(buffer,4096,"%02X",
...
# 1641 "coders/ps.c" 3 4
                                          ((void *)0)
# 1641 "coders/ps.c"
                                              )
              { (void) ThrowMagickException(exception,"coders/ps.c",__func__,(unsigned long) 1642,ResourceLimitError,"MemoryAllocationFailed", "`%s'",image->filename); if (image_info->adjoin != MagickFalse) while (image->previous != (Image *) 
# 1642 "coders/ps.c" 3 4
             ((void *)0)
# 1642 "coders/ps.c"
...
# 1657 "coders/ps.c"
              ...
              for (x=0; x < (ssize_t) preview_image->columns; x++)
              {
                ...
                if (luma >= (Quantum) (((Quantum) 65535.0)/2))
                  byte|=0x01;
                bit++;
                if (bit == 8)
                  {
...
# 1750 "coders/ps.c"
    ...
    (void) WriteBlobString(image,buffer);
    labels=(char **) 
# 1762 "coders/ps.c" 3 4
                    ((void *)0)
# 1762 "coders/ps.c"
                        ;
    value=GetImageProperty(image,"label",exception);
    if (value != (const char *) 
# 1764 "coders/ps.c" 3 4
                               ((void *)0)
# 1764 "coders/ps.c"
                                   )
      labels=StringToList(value);
    if (labels != (char **) 
# 1766 "coders/ps.c" 3 4
                           ((void *)0)
# 1766 "coders/ps.c"
                               )
      {
        for (i=0; labels[i] != (char *) 
# 1768 "coders/ps.c" 3 4
                                       ((void *)0)
# 1768 "coders/ps.c"
                                           ; i++)
        {
    ...
    pixel.alpha=(MagickRealType) ((Quantum) 0);
    index=(Quantum) 0;
    x=0;
                ...
                for (x=0; x < (ssize_t) image->columns; x++)
                {
                  if ((GetPixelRed(image,p) == ClampToQuantum(pixel.red)) &&
                  ...
                  else
                    {
                      if (x > 0)
                        {
                          { if ((image->alpha_trait != UndefinedPixelTrait) && (length != 0) && (GetPixelAlpha(image,p) == (Quantum) ((Quantum) 0))) { q=PopHexPixel(hex_digits,0xff,q); q=PopHexPixel(hex_digits,0xff,q); q=PopHexPixel(hex_digits,0xff,q); } else { q=PopHexPixel(hex_digits,ScaleQuantumToChar(ClampToQuantum(pixel.red)),q); q=PopHexPixel(hex_digits,ScaleQuantumToChar(ClampToQuantum(pixel.green)),q); q=PopHexPixel(hex_digits,ScaleQuantumToChar(ClampToQuantum(pixel.blue)),q); } q=PopHexPixel(hex_digits,(size_t) (((length) < (0xff)) ? (length) : (0xff)),q); };
                          if ((q-pixels+10) >= 80)
                            {
                              *q++='\n';
                              (void) WriteBlob(image,(size_t) (q-pixels),pixels);
...
# 1980 "coders/ps.c"
                ...
                for (x=0; x < (ssize_t) image->columns; x++)
                {
                  if ((image->alpha_trait != UndefinedPixelTrait) &&
                      (GetPixelAlpha(image,p) == (Quantum) ((Quantum) 0)))
                    {
...
# 2008 "coders/ps.c"
      ...
      else
        {
          ...
          {
            (void) FormatLocaleString(buffer,4096,"%02X%02X%02X\n",
...
# 2061 "coders/ps.c"
                ...
                q=PopHexPixel(hex_digits,(size_t) index,q);
                q=PopHexPixel(hex_digits,(size_t) (((length) < (0xff)) ? (length) : (0xff)),q);
                if ((q-pixels+6) >= 80)
                  {
                    *q++='\n';
                    (void) WriteBlob(image,(size_t) (q-pixels),pixels);
